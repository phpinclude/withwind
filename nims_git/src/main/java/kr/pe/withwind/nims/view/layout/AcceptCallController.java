...
public class AcceptCallController extends CommonController{
...
//		cbSearchRange.setItems(FXCollections.observableArrayList(TreeCodeManager.getInstance().getChildCodeList("NIMS_CODE","SEARCH_RANGE")));
		cbSearchRange.getItems().addAll("전체");
	...
	private void doSearch() {
		...
		try{
			...
			ArrayList<JSONObject> result = mainClass.doTaskExcute(CALL_TYPE.ACCEPT_PD_INFO,urlParam);
			...
			Platform.runLater(()->{
				DialogManager.showExceptionDialog(e, "에러", "제고정보 조회 실패", "내용을 확인해주세요.");
			});
		}
	}
